 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		_renderHeader: function() {
			...
			prevIcon.addClass('icon-chevron-thin-left');
			...
			nextIcon.addClass('icon-chevron-thin-right');
		...
		_renderBody: function() {
			...
			var monthsDivInner = $(document.createElement('div'));
			monthsDivInner.addClass('row');
			...
			for(var m = 0; m < 12; m++) {
				...
				monthsDivInner.append(monthDiv);
				
				monthsDiv.append(monthsDivInner);
			}
		...
		_openContextMenu: function(elt) {
			...
			for(var i = 0; i < events.length; i++) {
				...
				icon.addClass('icon-chevron-thin-right');
		...
		_renderContextMenuItems: function(parent, items, evt) {
			...
			for(var i = 0; i < items.length; i++) {
				if(!items[i].visible || items[i].visible(evt)) {
					...
					var icon = $(document.createElement('span'));
					icon.addClass('icon-chevron-thin-left');
