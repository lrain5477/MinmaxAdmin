<div class="form-group row">
    ...
    <div class="col-sm-10">
        ...
        <div class="custom-file">
            ...
            <label class="custom-file-label" id="{{ $id }}-label" for="{{ $id }}">{{ $filename == '' ? __('administrator.form.file.default_text') : $filename }}</label>
        </div>
        @if($filename != '')
        <div class="custom-control custom-checkbox">
            <input class="custom-control-input" type="checkbox" id="{{ $id }}-remove" />
            <label class="custom-control-label" for="{{ $id }}-remove">@lang('administrator.form.file.remove_file')</label>
        </div>
        @endif
    </div>
    @if($hint !== '')
    ...
    $(function() {
        $('#{{ $id }}').on('change', function() {
            ...
            if(limit > 0 && this.files.length > limit) {
                this.value = '';
                $('#{{ $id }}-label').text('{{ $filename == '' ? __('administrator.form.file.default_text') : $filename }}');
                swal('@lang('administrator.form.file.limit_title')', '@lang('administrator.form.file.limit_text', ['limit' => $limit])');
            } else {
                var nameList = '';
                for(var i = 0; i < this.files.length; i++) {
                    nameList += this.files[i] ? ((nameList === '' ? '' : ', ') + this.files[i].name) : '';
                }
                $('#{{ $id }}-label').text(nameList === '' ? '@lang('administrator.form.file.default_text')' : nameList);
            }
        });
        @if($filename != '')
        $('#{{ $id }}-remove').on('change', function() {
            if($(this).prop('checked')) {
                $('#{{ $id }}-origin').val('');
                $('#{{ $id }}-label').text('@lang('administrator.form.file.default_text')');
                $('#{{ $id }}-list').hide();
            } else {
                $('#{{ $id }}-origin').val('{{ $file }}');
