...
<template id="template-{{ $id }}">
    <tr>
        ...
        <td>DUMP_COST<input type="hidden" name="ProductItem[cost][DUMP_CURRENCY_KEY]" value="DUMP_COST" /></td>
        <td>DUMP_PRICE<input type="hidden" name="ProductItem[price][DUMP_CURRENCY_KEY]" value="DUMP_PRICE" /></td>
        <td class="text-center text-nowrap">
            <button class="btn btn-danger btn-sm repeat-remove" type="button"><i class="icon-cross"></i></button>
        </td>
    </tr>
    ...
    $(function () {
        ...
        $('#{{ $id }}')
            .on('click', '.repeat-add', function () {
                ...
                var currencyLable = $('#{{ $id }} table > tfoot .repeat-key > option:selected').text();
                ...
                if (currency !== '' && cost !== '' && price !== '') {
                    $('#{{ $id }} table > tbody').append(
                        $('#template-{{ $id }}').html()
                            .replace(/DUMP_CURRENCY_LABEL/ig, currencyLable)
