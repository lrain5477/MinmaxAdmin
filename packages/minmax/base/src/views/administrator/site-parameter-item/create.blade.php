...
@section('action-buttons')
    ...
    <a class="btn btn-sm btn-light" href="{{ langRoute("administrator.{$pageData->uri}.index", ['group' => request('group')]) }}" title="@lang('MinmaxBase::administrator.form.back_list')">
...
@section('forms')
    ...
    <fieldset id="baseFieldSet">
        ...
        {!! $modelPresenter->getFieldSelect($formData, 'group_id', ['required' => true, 'defaultValue' => request('group')]) !!}

        {!! $modelPresenter->getFieldText($formData, 'label', ['required' => true]) !!}

        {!! $modelPresenter->getFieldText($formData, 'value', ['size' => 4, 'hint' => true]) !!}
