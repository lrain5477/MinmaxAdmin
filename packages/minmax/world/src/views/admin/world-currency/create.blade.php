...
@section('action-buttons')
    @component('MinmaxBase::admin.layouts.right-links', ['languageActive' => $languageActive])
        @if($adminData->can('worldCurrencyShow'))
...
@section('forms')
    ...
    <fieldset id="baseFieldSet">
        ...
        {!! $modelPresenter->getFieldText($formData, 'options', ['required' => true, 'size' => 4, 'subColumn' => 'symbol']) !!}

        {!! $modelPresenter->getFieldText($formData, 'options', ['required' => true, 'size' => 4, 'subColumn' => 'native']) !!}

        {!! $modelPresenter->getFieldText($formData, 'options', ['required' => true, 'size' => 4, 'subColumn' => 'exchange']) !!}
