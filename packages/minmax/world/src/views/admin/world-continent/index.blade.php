...
@section('action-buttons')
    @component('MinmaxBase::admin.layouts.right-links')
        @if($adminData->can('worldContinentCreate'))
...
@inject('modelPresenter', 'Minmax\World\Admin\WorldContinentPresenter')

@section('grid-filter')
    @component('MinmaxBase::admin.layouts.grid.filter-keyword')
    <option value="title">@lang('MinmaxWorld::models.WorldContinent.title')</option>
    <option value="code">@lang('MinmaxWorld::models.WorldContinent.code')</option>
    @endcomponent

    @component('MinmaxBase::admin.layouts.grid.filter-equal')
    {!! $modelPresenter->getFilterSelection('active', 'searchActive', ['emptyLabel' => __('MinmaxWorld::models.WorldContinent.active')]) !!}
    @endcomponent
@endsection
